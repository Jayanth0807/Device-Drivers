...
static ssize_t f_read(struct file *filp,char *buff,size_t len,loff_t *off)
{
	...
	nbytes= len-copy_to_user(buff,k_buff,len);
	*off += nbytes;
	printk(KERN_INFO "writing to the user=%s,pos=%d\n",k_buff,(int)*off);
	return nbytes;
}
...
static ssize_t f_write(struct file *filp,const char *buff,size_t len,loff_t *off)
{
	...
	*off=0;
	...
	{
		temp = k_buff[i];
		k_buff[i] = k_buff[len-i-1];
		k_buff[len-i-1] = temp;
	}
	printk(KERN_INFO"while loop worked \n");
	*off += nbytes;
...
{
	printk(KERN_INFO"Entering the kernel region");
	if(alloc_chrdev_region(&dev,0,1,"jay_4")<0)
	{
